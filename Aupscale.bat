...
ffprobe.exe -v error -select_streams v -of default=noprint_wrappers=1:nokey=1 -show_entries stream=r_frame_rate "%file_path%" > fpout
set /p frame_rate= <fpout
del fpout
echo Encodiing audio from original video to Opus
mkdir "%file_name%-audio"
ffmpeg.exe -i "%file_path%" -c:a libopus -vbr on -b:a 192k "%file_name%-audio\audio.opus"
echo Extracting frames from video
ffmpeg.exe -i "%file_path%" -r %frame_rate% %frames_path%frame%%09d.png
echo Upscaling frames
%waifu2x_path_exe% -i %frames_path% -o %upscaled_path% -s %scale% -n %noise% -v
echo Rendering upscaled video
ffmpeg.exe -framerate %frame_rate% -pattern_type sequence -start_number 000000001 -i %upscaled_path%frame%%09d.png -i "%file_name%-audio\audio.opus" -c:a copy -shortest -c:v libx265 -crf 19 -preset slow -x265-params "limit-sao=1:bframes=8:psy-rd=1:aq-mode=3" "%Export_path%%file_name%.upscaled.mkv"
